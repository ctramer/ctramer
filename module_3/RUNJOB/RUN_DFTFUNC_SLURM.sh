...
if [ $DFTLEVEL -eq 1 ]; then
  ...
  if [ $PHASE -eq 0 ]; then
  cp -a DFT_FUNCTIONALS_SABINE/bnl.sh dft.sh
  ./dft.sh
  else
  cp -a DFT_FUNCTIONALS_SABINE/bnl-pcm.sh dft.sh
  ./dft.sh 
  fi
...
# For B3LYP DFT Functional
  ...
  if [ $PHASE -eq 0 ]; then
  cp -a DFT_FUNCTIONALS_SABINE/b3lyp.sh dft.sh
  ./dft.sh
  else
  cp -a DFT_FUNCTIONALS_SABINE/b3lyp-pcm.sh dft.sh
  ./dft.sh
  fi
...
# For wPBEh DFT Functional
  ...
  if [ $PHASE -eq 0 ]; then
  cp -a DFT_FUNCTIONALS_SABINE/wPBEh.sh dft.sh
  ./dft.sh
  else
  cp -a DFT_FUNCTIONALS_SABINE/wPBEh-pcm.sh dft.sh
  ./dft.sh
  fi
