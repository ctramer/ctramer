#!/bin/bash
amber_name=AMBER_MODULE_NAME
vmd_name=VMD_MODULE_NAME
module load amber_name
module load vmd_name
MOL1=MOLEC1
MOL2=MOLEC2
num_configs=CONF_NUM
cp ../../module_4/configs.dat configs.dat
cp -r ../../module_4/states .
for (( config=1; config<=$num_configs; config++)); do
	DONOR="EX";
	run=$(awk -v var="$config" 'NR==var {print $1}' configs.dat)
	frame=$(awk -v var="$config" 'NR==var {print $2}' configs.dat)
	index1=$(awk -v var="$config" 'NR==var {print $3}' configs.dat)
	index2=$(awk -v var="$config" 'NR==var {print $4}' configs.dat)
	...
	nohup vmd -dispdev none -parm7 ../../../module_2/AAADDD_${run}/AAADDD_${run}_new.prmtop \
		-crdbox ../../../module_2/AAADDD_${run}/AAADDD_${run}_new.mdcrd \
		-e extract_frame.tcl
